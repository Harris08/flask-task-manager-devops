...
SES_FROM     = os.environ.get('SES_FROM_EMAIL', 'haarisraja08@gmail.com')
...
class User(UserMixin):
    def __init__(self, id, name, email, role, is_first_login):
        ...
        self.email          = email
        self.role           = role
        self.is_first_login = bool(is_first_login)
...
def load_user(user_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, email, role, is_first_login FROM users WHERE id=%s",
                (user_id,)
            )
            row = cur.fetchone()
            if row:
...
def email_welcome(name, email, temp_password):
    ...
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1.5px solid #dbeafe;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:28px;">
        ...
        <p style="color:#334155;font-size:14px;">Your account has been created. Use the credentials below to sign in.</p>
...
def email_task_approved(emp_name, emp_email, task_title):
    html = f"""
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1.5px solid #86efac;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:28px;">
        ...
        <p style="color:#334155;font-size:14px;">Great work <strong>{emp_name}</strong>! Your task has been approved.</p>
...
def email_task_redo(emp_name, emp_email, task_title):
    ...
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1.5px solid #fda4af;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:28px;">
        ...
        <p style="color:#334155;font-size:14px;">Hi <strong>{emp_name}</strong>, your manager requested corrections on your task.</p>
...
def email_password_reset(email, reset_url):
    ...
    <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#fff;border:1.5px solid #dbeafe;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:28px;">
        ...
        <p style="color:#334155;font-size:14px;">Click below to reset your password. Link expires in <strong>30 minutes</strong>.</p>
...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id, name, email, password_hash, role, is_first_login FROM users WHERE email=%s",
                    (email,)
                )
                row = cur.fetchone()
        finally:
...
def change_password():
    if request.method == 'POST':
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE users SET password_hash=%s, is_first_login=0 WHERE id=%s",
...
def redo_task(task_id):
    ...
    flash('Task sent back for redo.', 'warning')
    return redirect(url_for('manager_dashboard'))
...
def create_employee():
    ...
    flash(f'Employee "{name}" created! Credentials sent to {email}.', 'success')
    return redirect(url_for('manager_dashboard'))
...
def complete_task(task_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id FROM tasks WHERE id=%s AND assigned_to=%s",
